...
const CONFIG = {
    ...
    heroes: {
        sf: {
            ...
            stateRows: { idle: 0, attack: 1, hurt: 2 },
            skills: {
                z: { name: "影压(近)", cost: 30, damage: 100, cd: 2000 },
                x: { name: "影压(中)", cost: 30, damage: 100, cd: 2000 },
                c: { name: "影压(远)", cost: 30, damage: 100, cd: 2000 },
                r: { name: "魂之挽歌", cost: 100, damage: 400, cd: 10000 }
            }
        },
        cm: {
            id: "cm",
            ...
            stateRows: { idle: 0, attack: 1, hurt: 2 },
            skills: {
                z: { name: "冰霜新星", cost: 40, damage: 80, effect: "slow", cd: 3000 },
                x: { name: "冰封禁制", cost: 60, damage: 120, effect: "freeze", cd: 5000 },
                c: { name: "辉煌光环", cost: 0, damage: 0, effect: "mana_regen", cd: 0 },
                r: { name: "极寒领域", cost: 150, damage: 500, cd: 15000 }
            }
        },
        sniper: {
            id: "sniper",
            ...
            stateRows: { idle: 0, attack: 1, hurt: 2 },
            skills: {
                z: { name: "散弹", cost: 50, damage: 70, effect: "aoe", cd: 4000 },
                x: { name: "爆头", cost: 0, damage: 30, effect: "passive", cd: 0 },
                c: { name: "瞄准", cost: 0, damage: 0, effect: "range", cd: 0 },
                r: { name: "暗杀", cost: 100, damage: 600, cd: 12000 }
            }
        }
    },
...
let state = {
    ...
    monster: null,
    difficulty: 1, // 默认难度
    cooldowns: { z: 0, x: 0, c: 0, r: 0 } // 技能冷却 (毫秒)
};
...
function init() {
    ...
    document.querySelectorAll('.diff-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            state.difficulty = parseFloat(btn.getAttribute('data-diff'));
        });
    });
...
function updateSkillUI(heroConfig) {
    ...
    document.getElementById('skill-z').innerText = `Z (${skills.z.name})`;
    document.getElementById('skill-x').innerText = `X (${skills.x.name})`;
    document.getElementById('skill-c').innerText = `C (${skills.c.name})`;
...
function loadLevel(levelIdx) {
    ...
    const level = Object.assign({}, CONFIG.levels[levelIdx]);
    
    // 应用难度倍率 (1为简单，1.5为普通，2.5为困难)
    // 简单模式下怪物HP大幅降低，攻击力也降低，让用户能轻松看到最终Boss
    level.hp = Math.floor(level.hp * (state.difficulty === 1 ? 0.3 : state.difficulty));
    level.attack = Math.floor(level.attack * (state.difficulty === 1 ? 0.4 : state.difficulty));
...
function useSkill(key) {
    ...
    // 检查蓝量和冷却
    if (state.heroMana >= cost && now >= state.cooldowns[key]) {
        state.heroMana -= cost;
        state.cooldowns[key] = now + (skill.cd || 0);
...
function updateUI() {
    ...
    if (level) {
        // 计算当前难度的总HP用于显示比例
        const totalHp = Math.floor(level.hp * (state.difficulty === 1 ? 0.5 : state.difficulty));
        const monsterHpPercent = (state.monsterHp / totalHp) * 100;
    ...
    keys.forEach(key => {
        ...
        const isReady = state.heroMana >= (skill.cost || 0) && now >= state.cooldowns[key];
        const el = document.getElementById(`skill-${key}`);
        if (el) {
            el.classList.toggle('active', isReady);
            
            // 移动端按钮也同步状态
            const mobileBtn = document.querySelector(`.control-btn[data-key="${key}"]`);
            if (mobileBtn) {
                mobileBtn.style.opacity = isReady ? "1" : "0.4";
                mobileBtn.style.filter = isReady ? "grayscale(0)" : "grayscale(1)";
            }
        }
    });
}
